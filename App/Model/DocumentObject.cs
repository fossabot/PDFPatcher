    ...
    public bool HasChildren
    {
        get
        {
            if (Type != PdfObjectType.Normal
                && (Type is PdfObjectType.Trailer or PdfObjectType.Pages or PdfObjectType.Page
                        or PdfObjectType.PageCommands or PdfObjectType.Hidden ||
                            ...
                            case PdfObjectType.Outline:
                                {
                                    ...
                                    if (pd != null)
                                    {
                                        ...
                                        {
                                            o.Add(
                                                new DocumentObject(OwnerDocument, this, Constants.Bookmark, or,
                                                    PdfObjectType.Outline)
                                                {
                                                    Description = pd.Contains(PdfName.TITLE)
                                                        ? pd.GetAsString(PdfName.TITLE).ToUnicodeString()
            ...
            case PdfObject.ARRAY:
                {
                    ...
                    int n = 0;
                    foreach (DocumentObject d in pd.ArrayList.Select(item =>
                                 new DocumentObject(OwnerDocument, this, (++n).ToText(), item)))
                    {
                        r[n - 1] = d;
                        PdfStructInfo i = PdfStructInfo.GetInfo(GetContextName(), d.Name);
                        if (i.Name != null && i.IsKeyObject)
    ...
    private void PopulateChildrenForSpecialObject()
    {
        ...
        switch (Type)
        {
            ...
            case PdfObjectType.Pages:
                {
                    PageRangeCollection r =
                        PageRangeCollection.Parse(ExtensiveObject as string, 1, pdf.NumberOfPages, true);
            ...
            case PdfObjectType.Hidden:
                {
                    ...
                    ExtensiveObject = ul;
                    List<DocumentObject> uo =
                        (from item in ul
                         let u = pdf.GetPdfObjectRelease(item)
                         where u != null
                         select new DocumentObject(OwnerDocument, this, item.ToText(), u)).ToList();
            ...
            case PdfPageCommandType.InlineImage:
                {
                    ...
                    {
                        o._Children.Add(new DocumentObject(OwnerDocument, o, PdfHelper.DecodeKeyName(ii.Key),
                            ii.Value));
                    }
