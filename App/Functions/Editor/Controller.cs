...
internal sealed class Controller
{
        ...
        IPdfInfoXmlProcessor processor)
    {
        ...
        if (r != null)
        {
            foreach (XmlElement i in r.Select(item => item as XmlElement)
                         .Where(i => i.ParentNode.Name == Constants.DocumentBookmark))
            {
    ...
    private static void ProcessItem(bool includeDescendant, IPdfInfoXmlProcessor processor,
        ISet<XmlElement> processedItems,
    ...
    internal void ConfigAutoBookmarkTextStyles(int level, TextInfo textInfo)
    {
        ...
        foreach (MuTextSpan span in textInfo.Spans)
        {
            ...
            if (s != null)
            {
                ...
                foreach (EditModel.AutoBookmarkStyle unused in Model.TitleStyles.Where(item =>
                             item.InternalFontName == s.Name && item.FontSize == fs))
                {
                    goto NEXT;
                }
