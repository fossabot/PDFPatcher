...
internal sealed class BookmarkSelectionCommand : IEditorCommand
{
    ...
    public BookmarkSelectionCommand(string command) => _command = command;
    ...
    public void Process(Controller controller, params string[] parameters)
    {
        ...
        switch (_command)
        {
            ...
            case "_CollapseChildren":
                foreach (XmlNode ci in b.GetSelectedElements(false)
                             .SelectMany(item => item.SubBookmarks.Cast<XmlNode>()))
                {
                    b.Collapse(ci);
                }
