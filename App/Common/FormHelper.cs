...
internal static class FormHelper
{
    ...
    public static bool IsEmptyOrTransparent(this Color color) => color.IsEmpty || color.A == 0;

    public static Point Round(this PointF point) => new(point.X.ToInt32(), point.Y.ToInt32());
    ...
    public static Point Transpose(this Point point, int x, int y) => new(point.X + x, point.Y + y);

    public static Point Transpose(this Point point, Point transpose) =>
        new(point.X + transpose.X, point.Y + transpose.Y);

    public static void SetIcon(this Form form, Bitmap bitmap) => form.Icon = Icon.FromHandle(bitmap.GetHicon());
    ...
    public static NumericUpDown SetValue(this NumericUpDown box, int value) => box.SetValue((decimal)value);

    public static NumericUpDown SetValue(this NumericUpDown box, float value) => box.SetValue((decimal)value);
    ...
    internal static DialogResult YesNoBox(string text) =>
        MessageBox.Show(text, Constants.AppName, MessageBoxButtons.YesNo, MessageBoxIcon.Question);

    internal static DialogResult YesNoCancelBox(string text) => MessageBox.Show(text, Constants.AppName,
        MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);

    public static bool ConfirmOKBox(this Control control, string text) =>
        MessageBox.Show(text, control.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) ==
        DialogResult.OK;

    public static bool ConfirmYesBox(this Control control, string text) =>
        MessageBox.Show(text, control.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) ==
        DialogResult.Yes;
}
