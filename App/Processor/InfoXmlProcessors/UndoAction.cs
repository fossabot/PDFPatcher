...
internal sealed class UndoActionGroup : IUndoAction
{
    ...
    public void AddElement(XmlElement target) => Add(new AddElementAction(target));
...
{
    public AddElementAction(XmlElement target) : base(target) => RefNode = target.NextSibling;
...
{
    public SetAttributeAction(XmlElement targeNode, string name, string value) : base(targeNode, name) => Value = value;
