...
internal abstract class BookmarkMatcher
{
    ...
    private sealed class XPathMatcher : BookmarkMatcher
    {
        ...
        internal XPathMatcher(string pattern) => _xpath = XPathExpression.Compile("*[" + pattern + "]");

        internal override bool Match(XmlElement item) => item.CreateNavigator().Matches(_xpath);
    ...
    {
        internal override bool Match(XmlElement item) => true;
