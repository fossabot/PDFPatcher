...
internal sealed class TextFilter : AutoBookmarkFilter
{
    ...
    public TextFilter(MatchPattern pattern) => _matcher = pattern.CreateMatcher();

    internal override bool Matches(AutoBookmarkContext context) =>
        _matcher.Matches(context.TextLine == null ? context.TextInfo.Text : context.TextLine.Text);
