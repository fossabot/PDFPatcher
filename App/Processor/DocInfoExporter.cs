...
internal sealed class DocInfoExporter
{
    ...
    internal void ExtractPageLinks(XmlWriter w)
    {
        ...
        for (int i = 1; i <= numPages; i++)
        {
            ...
            foreach (PdfDictionary annot in from item in arr
                                            where !item.IsNull()
                                            select (PdfDictionary)PdfReader.GetPdfObjectRelease(item)
                     into annot
                                            where PdfName.LINK.Equals(annot.Get(PdfName.SUBTYPE))
                                            select annot)
            {
